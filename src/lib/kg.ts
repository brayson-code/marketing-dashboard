...
export async function upsertEntity(
  ...
  const existingRows = await sql()`
    ...
    WHERE tenant_id = ${tenantId()} AND kind = ${kind} AND name = ${name}
  `;
  ...
  const inserted = await sql()`
    ...
    VALUES (${tenantId()}, ${kind}, ${name}, ${attributes ? jsonb(attributes) : null}, ${sourceAgent}, ${confidence})
    RETURNING *
  `;
...
export async function link(
  ...
  const existingRows = await sql()`
    ...
    WHERE tenant_id = ${tenantId()} AND from_id = ${fromE.id} AND to_id = ${toE.id} AND label = ${label}
  `;
  ...
  const inserted = await sql()`
    ...
    VALUES (${tenantId()}, ${fromE.id}, ${toE.id}, ${label}, ${attributes ? jsonb(attributes) : null}, ${sourceAgent}, ${confidence})
    RETURNING *
  `;
...
export async function neighborsOf(entityId: number): Promise<Neighbor[]> {
  const out = await sql()`
    ...
    WHERE r.tenant_id = ${tenantId()} AND r.from_id = ${entityId}
  ...
  const inn = await sql()`
    ...
    WHERE r.tenant_id = ${tenantId()} AND r.to_id = ${entityId}
