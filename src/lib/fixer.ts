...
Diagnose and propose a minimal fix.`;
  ...
  try {
    ...
    await updateIssue(issueId, {
      ...
      suggested_fix: prUrl ? `${proposal.summary}` : `${proposal.summary}\n\n${patchText}`,
      pr_url: prUrl,
    });
    await finishTask(taskId, { status: 'done', result: prUrl ? `Opened draft PR: ${prUrl}` : `Proposed patch (${proposal.files.length} file(s))`, inputTokens: totalInput, outputTokens: totalOutput });
