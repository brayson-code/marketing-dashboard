...
import { getIssue, getIssueEvents, updateIssue } from './observability';
...
Diagnose and propose a minimal fix.`;
  ...
  try {
    ...
    // Save the diagnosis + full proposed patch immediately so it's preserved
    // even if opening a PR fails (e.g. a read-only GitHub token).
    ...
    if (isGitHubConfigured()) {
      try {
        ...
        for (const f of proposal.files) {
          ...
          await putFile(f.path, f.new_content, branch, `fix: ${proposal.summary.slice(0, 60)} (issue ${issueId.slice(0, 8)})`, existing?.sha);
        }
        const pr = await openPullRequest({
          head: branch,
          title: `fix: ${issue.title.slice(0, 72)}`,
          draft: true,
          body: `**KeyWatch auto-fix (draft — review before merge)**\n\n**Issue:** ${issue.title}\n**Root cause:** ${proposal.root_cause}\n\n**Change:** ${proposal.summary}\n\nFiles changed:\n${proposal.files.map((f) => `- \`${f.path}\``).join('\n')}\n\n_Generated by the Fixer agent from a production error. Verify locally before merging._`,
        });
        prUrl = pr.url;
        status = 'in_review';
        // PR opened — trim the inline patch dump since it now lives in the PR.
        ...
        // proposed patch on the issue so it's still actionable.
