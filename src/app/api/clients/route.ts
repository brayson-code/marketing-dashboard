...
// NOTE: client decommission (revoke/delete) is deliberately NOT exposed here. A destructive,
// cross-tenant, irreversible action must not be reachable from a browser session (a hijacked
// HQ cookie could nuke client data). Instead:
//   • the web app can only REQUEST it → POST /api/clients/decommission-request (notifies the
//     operator out-of-band; zero destructive power), and
//   • the action itself runs from a trusted machine → scripts/decommission-client.ts
//     (service-role key, behind a confirmation). See ops/docs/security-audit-2026-06-18.md.
